...
const CartPage = () => {
  ...
  const updateQuantity = async (productId, quantity) => {
    ...
    try {
      const response = await handleFetch(
        "http://localhost:3000/api/cart/updateCartItems",
        "PUT",
        { productId, quantity }
      );
  ...
  const removeFromCart = async (productId) => {
    ...
    try {
      ...
      setCartProducts((prev) => prev.filter((item) => item.product._id !== productId));
    } catch (error) {
    ...
    <div className="flex flex-col lg:flex-row items-center lg:items-start justify-center p-4 lg:p-8 w-full">
      ...
      <div className="w-full lg:w-2/3">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-4">
          <h2 className="text-2xl font-bold">Cart ({cartProducts.length} items)</h2>
        ...
        {cartProducts.length > 0 ? (
          <ul className="space-y-4">
            {cartProducts.map((item) => (
              <li key={item.product._id} className="flex flex-col sm:flex-row items-center gap-4 p-4 border rounded-lg">
                <Image className="rounded-lg" src={item.product.img_url} width={80} height={80} alt={item.product.name} />
                <div className="flex flex-col sm:flex-row items-center justify-between w-full">
                  <p className="text-lg font-medium">{item.product.name}</p>
                  <div className="flex items-center space-x-2">
                    <button onClick={() => updateQuantity(item.product._id, item.quantity - 1)} className="px-2 py-1 border rounded">-</button>
                    <span>{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.product._id, item.quantity + 1)} className="px-2 py-1 border rounded">+</button>
                  </div>
                  <p className="text-sm text-gray-600 font-bold">Rs. {item.product.sale_price ? item.product.sale_price * item.quantity : item.product.actual_price * item.quantity}
                  </p>
      ...
      {cartProducts.length > 0 && (
        <div className="w-full lg:w-1/3 bg-gray-100 p-4 rounded-lg mt-6 lg:mt-0 lg:ml-8">
          <div className="flex justify-between mb-4 text-lg font-semibold">
            <span>Subtotal</span>
            <div className="flex items-center"><LiaRupeeSignSolid /> <span>{totalCartValue}</span></div>
          </div>
